...
describe('session HTML workflow', () => {
  test('notes are escaped structured cards and finalize creates pending review delta', async () => {
    ...
    await addNote(root, { sessionId: session.id, type: 'design-decision', title: '<Normalize>', body: 'Use adapters <script>alert(1)</script>', files: ['src/connectors/crunchbase.ts'] });
