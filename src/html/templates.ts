...
export function areaPage(area: AreaDefinition): string {
  ...
  return pageLayout(area.title, `<h1>${escapeHtml(area.title)}</h1><p>${escapeHtml(area.summary)}</p><section data-section="purpose"><h2>Purpose</h2><p>${escapeHtml(area.summary)}</p></section><section data-section="related-tasks"><h2>Related Tasks</h2><ul>${tasks}</ul></section><section data-section="related-decisions"><h2>Related Decisions</h2><ul>${decisions}</ul></section><section id="recent-memory" data-section="recent-memory"><h2>Recent implementation memory</h2><p class="muted">No finalized implementation memory yet.</p></section><section data-section="open-questions"><h2>Open Questions</h2><p class="muted">No open questions recorded.</p></section><p><a href="agent-context.html">Agent context</a></p>`, { ...area, type: 'area' });
}
